    ...
    return (
      <DrawerContentScrollView {...this.props} style={Style.container}>
        <View style={Style.sub_container}>
          <View style={{ }}>
            <TouchableOpacity style={Style.touchable_opacity}
              onPress={() => this.goTo('Notification') }>
              <View style={{ marginLeft: -30, marginVertical: 10 }}>
                <Text>Hello</Text>
              </View>
              <View style={{ justifyContent: 'center', marginHorizontal: 5, }}>
                <Text style={{fontSize: 15, color: 'black', fontWeight: 'bold' }}> {this.state.name}</Text>
              </View>
            </TouchableOpacity>
          </View>
          <View style={{ flex: 0.7 }}>
            <View>
              <TouchableOpacity style={Style.touchable_opacity} onPress={() => this.goTo('Home')}>
